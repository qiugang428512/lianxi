	...
	*/
	__proto._update=function(autoKey){
		...
		if (tEventArr && this._eventIndex < tEventArr.length){
			...
			if (tEventData.time >=this._player.playStart && tEventData.time <=this._player.playEnd&& tEventData.name !== "undefined"){
				...
				}else if (tEventData.time < this._player.playStart && this._playAudio && tEventData.audioValue && tEventData.audioValue!=="null"&& tEventData.audioValue !== "undefined" && (this._player.playEnd-this._player.currentPlayTime)> 0.1){
				this._eventIndex++;
	...
	__proto._preloadAnimation=function(animationIndex){
		if (!this._templet._preloadMap[animationIndex]) {
			this._player.currentAnimationClipIndex = animationIndex;
			var totalFrameCount = this.total;
			for (var indexCounter = 0; indexCounter < totalFrameCount; ++indexCounter) {
				this._index=this._clipIndex=this._player.currentKeyframeIndex=indexCounter;
				var tGraphics = null;
				if (this._aniMode==0){
					tGraphics=this._templet.getGrahicsDataWithCache(animationIndex,indexCounter)||this._createGraphics();
					if (tGraphics && this.graphics!=tGraphics){
						this.graphics=tGraphics;
					}
					}else if (this._aniMode==1){
					tGraphics=this._getGrahicsDataWithCache(animationIndex,indexCounter)|| this._createGraphics();
					if (tGraphics && this.graphics!=tGraphics){
						this.graphics=tGraphics;
					}
					}else{
			...
			this._templet._preloadMap[animationIndex] = true;
		}
	}
	/**
	*@private
