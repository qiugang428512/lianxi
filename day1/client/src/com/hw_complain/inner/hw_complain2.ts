...
export class hw_complain2 extends Laya.Sprite {
	...
	public constructor(gameid: number, gamename: string, iconurl: string, offsety: number = 0) {
		super();
		this.mouseEnabled = true;
		this.size(720, 1280);
		hw_complain_config.GameId = gameid;
		hw_complain_config.GameName = gamename;
		hw_complain_config.Iconurl = iconurl;
	...
	private createIcon(): void {
		...
		nametxt.x = 150;
		nametxt.y = (120 - 34) / 2;;//(this._icongroup.height - 34) / 2;
	}
			...
			window["wx"].chooseImage({
				...
				success: (res: any) => {
					...
					for (let i: number = toi; i < toi + filepath.length; i++) {
						...
						bit.loadImage(filepath[fileIndex++], Laya.Handler.create(this, () => {
